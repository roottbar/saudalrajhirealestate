odoo.define('account_over_due.OverDueFormRenderer', function (require) {
"use strict";
...
var FollowupFormRenderer = FormRenderer.extend({
    ...
    renderSavedSummary: function (text) {
        ...
        var span = $('<span />')
        if (!text) {
            span.html("<input type='text' class='o_input o_field_widget' name='summary' />");
        } else {
            span.text(text);
            span.html(span.html().replace(/\n/g, '<br>'))
        }
        this.$('.o_account_reports_no_print .o_account_report_summary').empty().append(span);
    },
    /**
