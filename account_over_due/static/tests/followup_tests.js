odoo.define('accountReports.OverDueTests', function (require) {
"use strict";

var FollowupFormView = require('accountReports.OverDueFormView');
var testUtils = require('web.test_utils');

const cpHelpers = testUtils.controlPanel;
var createView = testUtils.createView;

QUnit.module('OverDueViews', {
    beforeEach: function () {
        this.data = {
            res_partner: {
                fields: {
    ...
    QUnit.test('followup form view get_followup_informations', async function (assert) {
        ...
        var followupFormView = await createView({
            ...
            model: 'res_partner',
            data: this.data,
            arch: '<form><sheet><overdue/></sheet></form>',
            res_id: 9,
            res_ids: [9],
    ...
    QUnit.test('followup form view do it later', async function (assert) {
        ...
        var followupFormView = await createView({
            ...
            model: 'res_partner',
            data: this.data,
            arch: '<form js_class="over_due_form"><sheet><overdue/></sheet></form>',
            res_id: 9,
            res_ids: [9],
            mockRPC: function (route, args) {
