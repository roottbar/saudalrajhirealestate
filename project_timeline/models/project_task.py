...
class ProjectTask(models.Model):
    ...
    def _auto_init(self):
        ...
        cr.execute(
            ...
            "WHERE table_name=%s AND column_name='planned_date_start'",
            (table_name,)
        )
        if not cr.fetchone():
            cr.execute(
                "ALTER TABLE {} ADD COLUMN planned_date_start timestamp".format(table_name)
            )
            cr.execute(
                """
                UPDATE {}
        ...
        cr.execute(
            "SELECT column_name FROM information_schema.columns "
            "WHERE table_name=%s AND column_name='planned_date_end'",
            (table_name,)
        )
        if not cr.fetchone():
            cr.execute(
                "ALTER TABLE {} ADD COLUMN planned_date_end timestamp".format(table_name)
            )
            cr.execute(
                """
                UPDATE {}
