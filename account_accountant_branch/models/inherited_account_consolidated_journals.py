    ...
    def _get_lines(self, options, line_id=None):
        ...
        if line_id:
            ...
            if line_model == 'account':
                where_params += [
                    str(split_line_id[2])]  # We append the id of the parent journal in case of an account line
        ...
        for values in results:
            ...
            if self._need_to_unfold('journal_%s' % (current_journal,), options) and values[
            ...
            if self._need_to_unfold('account_%s_%s' % (values['account_id'], values['journal_id']), options):
                vals = {
                    'id': 'month_%s__%s_%s_%s' % (
                        values['journal_id'], values['account_id'], values['month'], values['yyyy']),
                    ...
                    'columns': [{'name': n} for n in
                                [self.format_value(values['debit']), self.format_value(values['credit']),
