...
class AccountAccount(models.Model):
    ...
    def compute_values(self):
        ...
        if self._context.get('target_currency_id'):
        ...
        for account in self:
            ...
            query = f'SELECT debit, credit, date, company_id FROM {tables} WHERE {where_clause}'
            ...
            for deb, cre, date, company_id in self.env.cr.fetchall():
                ...
                if target_currency:
                    deb = company_dict[company_id].currency_id._convert(deb, target_currency, company_dict[company_id], date)
                    cre = company_dict[company_id].currency_id._convert(cre, target_currency, company_dict[company_id], date)
                balance += deb - cre
                credit += cre
                debit += deb
            ...
            if context.get('show_initial_balance'):
                ...
                tables, where_clause, where_params = self.env['account.move.line'].with_context(context)._query_get()
                query2 = f'SELECT debit, credit, date, company_id FROM {tables} WHERE {where_clause}'
