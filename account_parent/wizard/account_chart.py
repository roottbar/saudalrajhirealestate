...
class OpenAccountChart(models.TransientModel):
    ...
    def build_domain_context(self, wiz_id=None, account_id=None):
        ...
        if account_id:
            ...
            if context.get('state') and context['state'] == 'posted':
    ...
    def line_data(self, level, parent_id, wiz_id=False, account=False):
        return {
            ...
            'code': account.code,
            'name': account.name,
            'ac_type': self._m2o_to_str(account.account_type),
    ...
    def get_at_accounts(self, at_data, context):
        ...
        if not at_data['atype']:
            # In Odoo 18, use account_type instead of user_type_id.internal_group
            internal_groups = at_data['internal_group']
            account_domain += [('account_type', 'in', self._get_account_types_for_groups(internal_groups))]
        else:
            account_domain += [('account_type', '=', self._get_account_type_by_id(at_data['id'] * -1))]
        return self.env['account.account'].sudo().with_context(context).search(account_domain)
    
    def _get_account_types_for_groups(self, internal_groups):
        """Map internal groups to Odoo 18 account types"""
        mapping = {
            'asset': ['asset_receivable', 'asset_cash', 'asset_current', 'asset_non_current', 'asset_prepayments', 'asset_fixed'],
        ...
        for group in internal_groups:
            result.extend(mapping.get(group, []))
        return result
    
    def _get_account_type_by_id(self, type_id):
        """Get account type by old type id - fallback to a default"""
        account_type = self.env['account.account.type'].browse(type_id)
        if account_type.exists():
            # Map old type to new account_type field values
            if account_type.internal_group == 'asset':
                return 'asset_current'
            elif account_type.internal_group == 'liability':
                return 'liability_current'
            elif account_type.internal_group == 'equity':
                return 'equity'
            elif account_type.internal_group == 'income':
                return 'income'
            elif account_type.internal_group == 'expense':
                return 'expense'
        return 'asset_current'  # fallback
