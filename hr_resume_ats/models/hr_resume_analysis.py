    ...
    def _extract_text_from_pdf(self, file_data):
        ...
        try:
            ...
            try:
                ...
                _logger.info(f"PDF reader created successfully. Number of pages: {len(pdf_reader.pages)}")
            except Exception as reader_error:
                _logger.error(f"Failed to create PDF reader: {str(reader_error)}")
                raise UserError(_("Failed to read PDF file. The file might be corrupted or in an unsupported format."))
            ...
            for page_num, page in enumerate(pdf_reader.pages):
                try:
                    ...
                    if page_text and page_text.strip():
                        text += page_text + '\n'
                        successful_pages += 1
                        _logger.debug(f"Successfully extracted text from page {page_num + 1}")
                    else:
                        _logger.warning(f"No text found on page {page_num + 1}")
                except Exception as page_error:
                    _logger.warning(f"Error extracting text from page {page_num + 1}: {str(page_error)}")
                    continue
            
            _logger.info(f"Text extraction completed. Successful pages: {successful_pages}/{len(pdf_reader.pages)}")
            
            extracted_text = text.strip()
            if not extracted_text:
                _logger.error("No text could be extracted from any page")
                raise UserError(_("No text could be extracted from the PDF. The file might be image-based, corrupted, or contain only images."))
            
            _logger.info(f"Successfully extracted {len(extracted_text)} characters from PDF")
            return extracted_text
            
        except UserError:
            raise
        except Exception as e:
            _logger.error(f"Unexpected error extracting text from PDF: {str(e)}")
            _logger.error(f"Error type: {type(e).__name__}")
            error_msg = str(e)
