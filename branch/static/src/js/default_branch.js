...
var SwitchBranchMenu = Widget.extend({
    ...
    willStart: function () {
        ...
        var user_branches_companies = session.user_branches_companies;
        var allowed_companys = session.user_context.allowed_company_ids

        for(var index in user_branches_companies) {
              var user_branches_company = user_branches_companies[index];
              if(!(allowed_companys.includes(user_branches_company[0]))){
                    if(user_branches_company[1].length != 0){
                        this.user_branches = this.user_branches.filter(function(branch) {
                            return !(user_branches_company[1].includes(branch[0]))
                        });
                    }
              }
        }
        ...
        if (_.find(this.user_branches, function (branch) {
            ...
            this.current_branch_name = this.user_branches.length != 0  && this.user_branches[0][1] || ""
        }
