...
class MrpProduction(models.Model):
    ...
    def _prepare_production_task(self, production):
        ...
        task_name = "%s::%s%s" % (
            ...
            product.name)
        task_descr = _("""Manufacturing Order: %s
...
Planned Date: %s""") % (
            ...
            production.bom_id.name, production.date_planned_start)
        return {
            'mrp_production_id': production.id,
            'user_ids': [(6, 0, [production.user_id.id])] if production.user_id else [],
            'name': task_name,
    ...
    def unlink(self):
        ...
        if not tasks.mapped('timesheet_ids'):
            child_tasks = tasks.filtered(lambda x: x.child_ids)
            child_tasks.unlink()
...
class MrpWorkorder(models.Model):
    ...
    timesheet_ids = fields.One2many(
        comodel_name="account.analytic.line", inverse_name="workorder",
        string="Timesheets")