...
class KsListExport(http.Controller):
    ...
    def base(self, data):
        ...
        if ks_export_boolean:
            ...
            if not ks_timezone:
                ...
                if ks_tzone:
                    ...
                    with open('/etc/timezone') as tz_file:
                        ks_timezone = tz_file.read().strip()
                    try:
            ...
            item = item.with_context(ksDateFilterStartDate=ks_start_date,
                                     ksDateFilterEndDate=ks_end_date,
                                     ksDateFilterSelection=ksDateFilterSelection)

            if item._context.get('ksDateFilterSelection', False):
                if ksDateFilterSelection == 'l_custom':
                    item = item.with_context(
                        ksDateFilterStartDate=ks_start_date,
                        ksDateFilterEndDate=ks_end_date,
                ...
                item = item.with_context(
                    ksDateFilterStartDate=item.ks_dashboard_ninja_board_id.ks_dashboard_start_date,
                    ksDateFilterEndDate=item.ks_dashboard_ninja_board_id.ks_dashboard_end_date,
            ...
            if list_data['type'] == 'ungrouped':
                ...
                ks_query_result = item.ks_get_list_query_result(
                    ks_query, query_start_date, query_end_date,
                    ks_offset=0, ks_export_all=True
                )
        ...
        for dataset in list_data['data_rows']:
            if list_data['type'] != 'grouped':
                for count, index in enumerate(dataset['ks_column_type']):
                    if index == 'datetime' and dataset['data'][count]:
            ...
            for ks_count, val in enumerate(dataset['data']):
                if isinstance(val, (float, int)):
                    try:
                        ks_precision = item.sudo().env.ref(
                            'ks_dashboard_ninja.ks_dashboard_ninja_precision'
                        ).digits
                    except Exception:
                        ks_precision = 2
