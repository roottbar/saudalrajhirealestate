...
class RentUnitsReportWizard(models.TransientModel):
    ...
    # إضافة الحقول الجديدة للمصروفات والإيرادات الإجمالية
    company_ids = fields.Many2many('res.company', string='الشركات', default=lambda self: self.env.company)
    analytic_account_ids = fields.Many2many('account.analytic.account', string='مراكز التكلفة')
    group_id = fields.Many2one('account.analytic.group', string='مجموعة تحليلية')
    analytic_account_id = fields.Many2one('account.analytic.account', string='حساب تحليلي')
    company_currency_id = fields.Many2one('res.currency', related='company_id.currency_id', string='عملة الشركة')
