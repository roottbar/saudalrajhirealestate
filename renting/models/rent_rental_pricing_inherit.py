...
class RentalPricing(models.Model):
    ...
    unit = fields.Selection(units, string='Unit', required=True, default='day')
    ...
    def _compute_price(self, duration, unit):
        ...
        if unit != self.unit:
            ...
            converted_duration = math.ceil(
                (duration * PERIOD_RATIO[unit]) / (self.duration * PERIOD_RATIO[self.unit])
            )
        else:
    ...
    def _compute_duration_vals(self, pickup_date, return_date):
        """Compute duration values between two datetimes."""
