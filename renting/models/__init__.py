# -*- coding: utf-8 -*-

from . import rent_account_move_inherit
from . import rent_config_property_maintenance_statuses
from . import rent_config_property_maintenance_types
from . import rent_config_property_purposes
from . import rent_config_property_types
from . import rent_config_unit_finishes
from . import rent_config_unit_maintenance_types
from . import rent_config_unit_maintenance_statuses
from . import rent_config_unit_overlooks
from . import rent_config_unit_purposes
from . import rent_config_unit_types

# from . import rent_maintenance_pivot_report

from . import rent_product_inherit
from . import rent_property
from . import rent_property_elevator
from . import rent_property_maintenance
# Commented out rental.pricing inheritance as this model doesn't exist in Odoo 18
# from . import rent_rental_pricing_inherit
# Commented out rental.wizard inheritance as this model doesn't exist in Odoo 18
# from . import rent_rental_wizard_inherit
from . import rent_res_partner_inherit
from . import rent_sale_advance_payment_inv_inherit
from . import rent_sale_order_inherit
from . import rent_unit_maintenance
from . import rent_account_asset_inherit
from . import rent_sale_invoices
from . import rent_units_report_wizard
