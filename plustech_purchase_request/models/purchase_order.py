...
class PurchaseOrder(models.Model):
    ...
    def _plustech_purchase_request_confirm_message_content(self, request, request_dict=None):
        self.ensure_one()
        if not request_dict:
            request_dict = {}
    ...
    def _plustech_purchase_request_confirm_message(self):
        request_obj = self.env["purchase.request"]
        for po in self:
            requests_dict = {}
            for line in po.order_line:
                for request_line in line.sudo().plustech_purchase_request_lines:
            ...
            for request_id in requests_dict:
                ...
                message = po._plustech_purchase_request_confirm_message_content(
    ...
    def _plustech_purchase_request_line_check(self):
        for po in self:
            for line in po.order_line:
                for request_line in line.plustech_purchase_request_lines:
    ...
    def button_confirm(self):
        self._plustech_purchase_request_line_check()
        res = super(PurchaseOrder, self).button_confirm()
        self._plustech_purchase_request_confirm_message()
        return res
    ...
    def unlink(self):
        ...
        for rec in self:
            for alloc in (
                rec.order_line.mapped("plustech_purchase_request_lines")
                .mapped("plustech_purchase_request_allocation_ids")
...
class PurchaseOrderLine(models.Model):
    ...
    plustech_purchase_request_lines = fields.Many2many(
        comodel_name="purchase.request.line",
        relation="plustech_purchase_request_purchase_order_line_rel",
        column1="purchase_order_line_id",
        column2="plustech_purchase_request_line_id",
        readonly=True,
        copy=False,
    )

    plustech_purchase_request_allocation_ids = fields.One2many(
    ...
    def action_open_request_line_tree_view(self):
        ...
        request_line_ids = []
        for line in self:
            request_line_ids += line.plustech_purchase_request_lines.ids
    ...
    def _prepare_stock_moves(self, picking):
        ...
        for v in val:
            ...
            v["plustech_purchase_request_allocation_ids"] = all_list
        return val
    ...
    def update_service_allocations(self, prev_qty_received):
        for rec in self:
            ...
            for alloc in allocation:
                ...
                if not qty_left:
                    alloc.plustech_purchase_request_line_id._compute_qty()
                    break
                ...
                message_data = self._prepare_request_message_data(
                    alloc, alloc.plustech_purchase_request_line_id, allocated_product_qty
                )
                message = self._plustech_purchase_request_confirm_done_message_content(
                    message_data
                )
                alloc.plustech_purchase_request_line_id.request_id.message_post(
                    body=message, subtype_id=self.env.ref("mail.mt_comment").id
                )
    ...
    @api.model
    def _plustech_purchase_request_confirm_done_message_content(self, message_data):
