...
class PurchaseRequestLine(models.Model):
    ...
    purchase_lines = fields.Many2many(
        ...
        column1="plustech_purchase_request_line_id",
    ...
    move_dest_ids = fields.One2many(
        ...
        inverse_name="created_plustech_purchase_request_line_id",
    ...
    plustech_purchase_request_allocation_ids = fields.One2many(
        ...
        inverse_name="plustech_purchase_request_line_id",
    ...
    @api.depends(
        "plustech_purchase_request_allocation_ids",
        "plustech_purchase_request_allocation_ids.stock_move_id.state",
        "plustech_purchase_request_allocation_ids.stock_move_id",
        "plustech_purchase_request_allocation_ids.purchase_line_id",
        "plustech_purchase_request_allocation_ids.purchase_line_id.state",
        "request_id.state",
        "product_qty",
    )
    ...
    def _compute_qty(self):
        for request in self:
            done_qty = sum(
                request.plustech_purchase_request_allocation_ids.mapped("allocated_product_qty")
            )
            open_qty = sum(
                request.plustech_purchase_request_allocation_ids.mapped("open_product_qty")
            )
    ...
    @api.depends(
        ...
        "plustech_purchase_request_allocation_ids.stock_move_id.state",
        "plustech_purchase_request_allocation_ids.stock_move_id",
        "plustech_purchase_request_allocation_ids.purchase_line_id.order_id.state",
        "plustech_purchase_request_allocation_ids.purchase_line_id",
    )
    def _compute_qty_cancelled(self):
        for request in self:
            if request.product_id.type != "service":
                qty_cancelled = sum(
                    request.mapped("plustech_purchase_request_allocation_ids.stock_move_id")
                    .filtered(lambda sm: sm.state == "cancel")
                    .mapped("product_qty")
                )
            else:
                qty_cancelled = sum(
                    request.mapped("plustech_purchase_request_allocation_ids.purchase_line_id")
                    .filtered(lambda sm: sm.state == "cancel")
                    .mapped("product_qty")
                )
            ...
            if request.product_uom_id:
                request.qty_cancelled = (
                    ...
                    if request.plustech_purchase_request_allocation_ids
                    else 0
                )
            else:
    ...
    def _calc_new_qty(self, request_line, po_line=None, new_pr_line=False):
        ...
        if new_pr_line:
            ...
            for prl in po_line.plustech_purchase_request_lines:
                for alloc in prl.plustech_purchase_request_allocation_ids:
    ...
    def action_show_details(self):
        self.ensure_one()
        view = self.env.ref("plustech_purchase_request.view_plustech_purchase_request_line_details")
        return {
