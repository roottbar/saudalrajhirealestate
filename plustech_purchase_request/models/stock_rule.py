...
class StockRule(models.Model):
    ...
    @api.model
    def _prepare_plustech_purchase_request_line(self, request_id, procurement):
    ...
    @api.model
    def _prepare_plustech_purchase_request(self, origin, values):
    ...
    def is_create_plustech_purchase_request_allowed(self, procurement):
        """
        ...
        return (
            ...
            and procurement[0].product_id.plustech_purchase_request
        )

    def _run_buy(self, procurements):
        indexes_to_pop = []
        for i, procurement in enumerate(procurements):
            if self.is_create_plustech_purchase_request_allowed(procurement):
                self.create_plustech_purchase_request(procurement)
    ...
    def create_plustech_purchase_request(self, procurement_group):
        """
        Create a purchase request containing procurement order product.
        """
        procurement = procurement_group[0]
        rule = procurement_group[1]
        plustech_purchase_request_model = self.env["purchase.request"]
        plustech_purchase_request_line_model = self.env["purchase.request.line"]
        cache = {}
        ...
        if domain in cache:
            ...
            cache[domain] = pr
        if not pr:
            request_data = rule._prepare_plustech_purchase_request(
                procurement.origin, procurement.values
            )
            pr = plustech_purchase_request_model.create(request_data)
            cache[domain] = pr
        ...
        # Create Line
        request_line_data = rule._prepare_plustech_purchase_request_line(pr, procurement)
        plustech_purchase_request_line_model.create(request_line_data)
