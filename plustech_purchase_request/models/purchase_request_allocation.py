...
class PurchaseRequestAllocation(models.Model):
    ...
    plustech_purchase_request_line_id = fields.Many2one(
    ...
    company_id = fields.Many2one(
        ...
        readonly=True,
        related="plustech_purchase_request_line_id.request_id.company_id",
        store=True,
        index=True,
    )
    ...
    product_id = fields.Many2one(
        ...
        related="plustech_purchase_request_line_id.product_id",
        readonly=True,
    )
    product_uom_id = fields.Many2one(
        string="UoM",
        comodel_name="uom.uom",
        related="plustech_purchase_request_line_id.product_uom_id",
        readonly=True,
        required=True,
    )
    ...
    def _notify_allocation(self, allocated_qty):
        ...
        for allocation in self:
            request = allocation.plustech_purchase_request_line_id.request_id
            po_line = allocation.purchase_line_id
            message_data = self._prepare_message_data(po_line, request, allocated_qty)
            message = self._plustech_purchase_request_confirm_done_message_content(message_data)
