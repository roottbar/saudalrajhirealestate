...
class PurchaseRequest(models.Model):
    ...
    assigned_to = fields.Many2one(
        ...
        domain=lambda self: [
            (
                "groups_id",
                "in",
                self.env.ref("plustech_purchase_request.group_plustech_purchase_request_manager").id,
            )
        ],
        index=True,
    )
    ...
    def _compute_move_count(self):
        for rec in self:
            rec.move_count = len(
                rec.mapped("line_ids.plustech_purchase_request_allocation_ids.stock_move_id")
            )
    ...
    def action_view_stock_picking(self):
        ...
        action["context"] = {}
        lines = self.mapped(
            "line_ids.plustech_purchase_request_allocation_ids.stock_move_id.picking_id"
        )
        if len(lines) > 1:
            action["domain"] = [("id", "in", lines.ids)]
        elif lines:
    ...
    def action_view_plustech_purchase_request_line(self):
        action = (
            self.env.ref("plustech_purchase_request.plustech_purchase_request_line_form_action")
            .sudo()
            .read()[0]
        )
        ...
        if len(lines) > 1:
            ...
            action["views"] = [
                (self.env.ref("plustech_purchase_request.plustech_purchase_request_line_form").id, "form")
            ]
