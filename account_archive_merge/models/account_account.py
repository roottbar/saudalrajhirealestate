...
class AccountAccount(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None):
        # إخفاء الحسابات المؤرشفة افتراضياً
        context = self._context or {}
        if not context.get('show_archived_accounts', False):
    ...
    def action_archive_account(self):
        ...
        for account in self:
            ...
            if account.child_ids.filtered(lambda c: not c.is_archived):
                raise UserError(_('Cannot archive account %s because it has active child accounts.') % account.name)
